...
// ValidateAll checks the field values on RuleSet with the rules defined in the
// proto definition for this message. If any rules are violated, the result is
// a list of violation errors wrapped in RuleSetMultiError, or nil if none found.
func (m *RuleSet) ValidateAll() error {
	return m.validate(true)
}

func (m *RuleSet) validate(all bool) error {
	if m == nil {
		return nil
	}
		...
		if all {
			...
			case interface{ ValidateAll() error }:
				if err := v.ValidateAll(); err != nil {
					errors = append(errors, RuleSetValidationError{
	...
	if len(errors) > 0 {
		return RuleSetMultiError(errors)
	}

	return nil
}

// RuleSetMultiError is an error wrapping multiple validation errors returned
// by RuleSet.ValidateAll() if the designated constraints aren't met.
type RuleSetMultiError []error

// Error returns a concatenation of all the error messages it wraps.
func (m RuleSetMultiError) Error() string {
	var msgs []string
	for _, err := range m {
...
func (m RuleSetMultiError) AllErrors() []error { return m }
